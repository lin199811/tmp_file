...
class NuScenesDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if self.dataset_cfg.get('CLASS_MAP', None) is not None:
            def rename(x):
                if self.dataset_cfg.CLASS_MAP.get(x, None) is not None:
                    return self.dataset_cfg.CLASS_MAP[x]
                else:
                    return x
            for i in range(self.infos.__len__()):
                self.infos[i]['gt_names'] = np.array([rename(x) for x in self.infos[i]['gt_names']])

        if (
            dataset_cfg.get("SAMPLED_INTERVAL", None) is not None
            and dataset_cfg.SAMPLED_INTERVAL[self.mode] > 1
        ):
            si = dataset_cfg.SAMPLED_INTERVAL[self.mode]
            self.infos = self.infos[::si]
            self.logger.info(
                f"Total sampled samples for NuScenes dataset: {self.infos.__len__()}"
            )
    ...
    def __getitem__(self, index):
        ...
        if 'gt_boxes' in info:
            ...
            if (rot := self.dataset_cfg.get("ROTATE_Z", None)) is not None:
                input_dict["gt_boxes"][:, 0:3] = common_utils.rotate_points_along_z(
                    input_dict["gt_boxes"][None, :, 0:3], np.array(rot)
                )[0]
                input_dict["gt_boxes"][:, 6] += np.array(rot)
