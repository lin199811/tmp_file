...
#             print('#####info_path:', info_path)
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        def waymo_fast_eval(eval_det_annos, eval_gt_annos):
            from .waymo_fast_eval import OpenPCDetWaymoDetectionMetricsEstimator
            eval = OpenPCDetWaymoDetectionMetricsEstimator()
        ...
        elif kwargs['eval_metric'] == 'fast_waymo':
            ap_result_str, ap_dict = waymo_fast_eval(eval_det_annos, eval_gt_annos)
        else:
...
def create_waymo_infos_for_test(dataset_cfg, class_names, data_path, save_path,
                       raw_data_tag='raw_data_for_test', processed_data_tag='waymo_processed_data',
                       workers=min(16, multiprocessing.cpu_count()), update_info_only=False):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
        training=False, logger=common_utils.create_logger()
...
#     print('######ROOT_DIR:', ROOT_DIR)
    ...
    if args.func == 'create_waymo_infos':
    ...
    elif args.func == 'create_waymo_infos_for_test':
        try:
            yaml_config = yaml.safe_load(open(args.cfg_file), Loader=yaml.FullLoader)
        except:
            yaml_config = yaml.safe_load(open(args.cfg_file))
        dataset_cfg = EasyDict(yaml_config)
        dataset_cfg.PROCESSED_DATA_TAG = args.processed_data_tag
        create_waymo_infos_for_test(
            dataset_cfg=dataset_cfg,
            class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
            data_path=ROOT_DIR / 'data' / 'waymo',
            save_path=ROOT_DIR / 'data' / 'waymo',
            raw_data_tag='raw_data_for_test',
            processed_data_tag=args.processed_data_tag,
            update_info_only=args.update_info_only
        )
