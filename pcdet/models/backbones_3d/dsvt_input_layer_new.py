...
from pcdet.models.model_utils.dsvt_utils import PositionEmbeddingLearned,get_inner_win_inds_pytroch
...
class DSVTInputLayer(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        # voxel_info['batch_size'] = voxel_coors[:,0].max() + 1
    ...
    def get_set(self, voxel_info, stage_id):
        ...
        set_voxel_inds_shift0, batch_of_set_index_shift0 = self.get_set_single_shift(batch_win_inds_shift0, stage_id, shift_id=0, coors_in_win=coors_in_win_shift0, coords=voxel_info[f'voxel_coors_stage{stage_id}'])
        voxel_info[f'set_voxel_inds_stage{stage_id}_shift0'] = set_voxel_inds_shift0
        voxel_info[f'set_of_batch_inds_stage{stage_id}_shift0'] = batch_of_set_index_shift0
        # compute key masks, voxel duplication must happen continuously: (zliu: deal with empty voxels !!!!)
        ...
        set_voxel_inds_shift1, batch_of_set_index_shift1 = self.get_set_single_shift(batch_win_inds_shift1, stage_id, shift_id=1, coors_in_win=coors_in_win_shift1, coords=voxel_info[f'voxel_coors_stage{stage_id}'])
        voxel_info[f'set_voxel_inds_stage{stage_id}_shift1'] = set_voxel_inds_shift1
        voxel_info[f'set_of_batch_inds_stage{stage_id}_shift1'] = batch_of_set_index_shift1
    ...
    def get_set_single_shift(self, batch_win_inds, stage_id, shift_id=None, coors_in_win=None,coords=None):
        ...
        ########################### get the batch index for each set!
        batch_size = coords[:,0].max()+1
        batch_index = coords[:,0]
        batch_of_set_index_list = -torch.ones_like(set_win_inds)
        for k in range(batch_size):
            batch_k_start = contiguous_win_inds[batch_index==k].min()
            batch_k_end = contiguous_win_inds[batch_index==k].max()
            valid_batch_k_index = (set_win_inds>=batch_k_start) & (set_win_inds<=batch_k_end)
            batch_of_set_index_list[valid_batch_k_index] = k
            # batch_k_num = batch_k_end - batch_k_start+1
            # k_indexes = k * torch.ones((batch_k_num))
            # batch_of_set_index_list.append(k_indexes)
        assert batch_of_set_index_list.shape[0]==set_win_inds.shape[0]
        assert (batch_of_set_index_list==-1).sum()==0
        # batch_of_set_index = torch.cat(batch_of_set_index_list)
        ###########################
        ...
        # region: sort order
        contiguous_win_inds_sorty = contiguous_win_inds[order2]
        inner_voxel_inds = get_inner_win_inds_pytroch(contiguous_win_inds_sorty)
        global_voxel_inds = contiguous_win_inds_sorty * max_voxel + inner_voxel_inds
        _, order1 = torch.sort(global_voxel_inds)
        ...
        # region: sort order
        contiguous_win_inds_sortx = contiguous_win_inds[order2]
        inner_voxel_inds = get_inner_win_inds_pytroch(contiguous_win_inds_sortx)
        global_voxel_inds = contiguous_win_inds_sortx * max_voxel + inner_voxel_inds
        _, order1 = torch.sort(global_voxel_inds)
        # endregion: sort order
        ...
        return all_set_voxel_inds, batch_of_set_index_list
    ...
    #
    #     # compution of Eq.3 in 'DSVT: Dynamic Sparse Voxel Transformer with Rotated Sets' - https://arxiv.org/abs/2301.06051,
    #     # for each window, we can get voxel indexs belong to different sets.  ## deal with empty voxel!!!!!
    #     offset_idx = set_inds_in_win[:,None].repeat(1, voxel_num_set) * voxel_num_set
    #     base_idx = torch.arange(0, voxel_num_set, 1, device=device)
    #     base_select_idx = offset_idx + base_idx
    #     base_select_idx = base_select_idx * voxelnum_per_win[set_win_inds][:,None] # voxel number for each window!
    #     base_select_idx = base_select_idx.double() / (setnum_per_win[set_win_inds] * voxel_num_set)[:,None].double() ####################
    #     base_select_idx = torch.floor(base_select_idx)
    #     # obtain unique indexs in whole space
    #     select_idx = base_select_idx
