...
class PillarRes18BackBone_one_stride(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.conv2 = spconv.SparseSequential(
            # [468, 468] <- [468, 468]
            block(32, 64, 3, norm_fn=norm_fn, stride=1, padding=1, indice_key='spconv2', conv_type='spconv'),
            SparseBasicBlock(64, 64, norm_fn=norm_fn, indice_key='res2'),
            SparseBasicBlock(64, 64, norm_fn=norm_fn, indice_key='res2'),
        )
        ...
        self.conv3 = spconv.SparseSequential(
            # [800, 704] <- [400, 352]
            block(64, 128, 3, norm_fn=norm_fn, stride=1, padding=1, indice_key='spconv3', conv_type='spconv'),
            SparseBasicBlock(128, 128, norm_fn=norm_fn, indice_key='res3'),
            SparseBasicBlock(128, 128, norm_fn=norm_fn, indice_key='res3'),
        )
        ...
        # self.conv4 = spconv.SparseSequential(
        #     # [400, 352] <- [200, 176]
        #     block(128, 256, 3, norm_fn=norm_fn, stride=1, padding=1, indice_key='spconv4', conv_type='spconv'),
        #     SparseBasicBlock(256, 256, norm_fn=norm_fn, indice_key='res4'),
        #     SparseBasicBlock(256, 256, norm_fn=norm_fn, indice_key='res4'),
        # )
        ...
        self.conv5 = nn.Sequential(
            # [200, 176] <- [100, 88]
            dense_block(128, 192, 3, norm_fn=norm_fn, stride=1, padding=1),
            BasicBlock(192, 192, norm_fn=norm_fn),
            BasicBlock(192, 192, norm_fn=norm_fn),
        )

        self.num_point_features = 192
        self.num_bev_features = 192 ###############
        self.backbone_channels = {
            'x_conv1': 32,
            'x_conv2': 64,
            'x_conv3': 128,
