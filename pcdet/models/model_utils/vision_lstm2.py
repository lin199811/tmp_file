...
class xLSTM_Block(nn.Module):
    def __init__(self, d_model, drop_path=0.0, conv_kind='causal1d', conv_kernel_size=4, proj_bias=True, norm_bias=True, with_cp=False,seqlens=None, **kwargs):
        super().__init__()
    # def __init__(self, d_model, drop_path=0.0, conv_kind='causal1d', conv_kernel_size=4, proj_bias=True, norm_bias=True, with_cp=False,seqlens=None, **kwargs):
    #     super().__init__()
    ...
    def forward(self, x):
        if self.with_cp:
            x = cp(self.rowwise_from_top_left, x)
            x = cp(self.rowwise_from_bot_right, x)
        else:
            x = self.rowwise_from_top_left(x)
            x = self.rowwise_from_bot_right(x)
        return x
