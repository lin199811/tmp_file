...
#         print('####use_dual_form:', use_dual_form)
...
class TTTBlock(nn.Module):
    def __init__(self, d_model, n_head, layer_id, use_gate, drop_path, **kwargs):
        super(TTTBlock, self).__init__()
        self.bos_token_id = 1 # bos_token_id # 1
        self.conv_kernel = 4 # conv_kernel # 4
        self.eos_token_id = 2 # eos_token_id # 2
        self.hidden_act = "silu" # hidden_act # "silu"
        self.hidden_size = d_model # hidden_size # 2048
        self.initializer_range = 0.02 # initializer_range # 0.02
        self.intermediate_size = d_model * 2 # intermediate_size # 5504
        self.mini_batch_size = 16 # mini_batch_size # 16 as default
        self.model_type = "ttt"
        self.num_attention_heads = n_head # num_attention_heads # 32 as default
        # self.num_hidden_layers = num_hidden_layers # 24 as default
        self.pre_conv = False # pre_conv # False as default
        self.rms_norm_eps = 1e-6 # rms_norm_eps # 1e-6 as default
        self.rope_theta = 10000.0 # rope_theta # 10000.0 as default
        self.scan_checkpoint_group_size = 0 # scan_checkpoint_group_size # 0
        self.share_qk = False # share_qk # False as default
        self.ttt_base_lr = 1 # ttt_base_lr # 1
        self.ttt_layer_type = "linear"
        # self.vocab_size = vocab_size
        # self.use_cache = use_cache
        self.use_gate = use_gate # False as default
        ...
        self.ttt_config = TTTConfig(
            ...
            scan_checkpoint_group_size=self.scan_checkpoint_group_size,
            share_qk=self.share_qk,
            ttt_base_lr=self.ttt_base_lr,
            ttt_layer_type=self.ttt_layer_type,
            # use_cache=self.use_cache,
            use_gate=self.use_gate,
        )
        
        self.seq_modeling_block = TTTLinear(config=self.ttt_config, layer_idx=layer_id)
        self.mlp = SwiGluMLP(self.ttt_config)
        if self.pre_conv:
            self.conv = Conv(self.ttt_config, layer_id)
            
        self.seq_norm = RMSNorm(self.ttt_config.hidden_size, eps=self.ttt_config.rms_norm_eps)
        self.ffn_norm = RMSNorm(self.ttt_config.hidden_size, eps=self.ttt_config.rms_norm_eps)
    ...
    def forward(
        ...
        #     cache_params=cache_params,
        # )
        hidden_states = self.seq_modeling_block(hidden_states, attention_mask, position_ids, cache_params) #cp(self.seq_modeling_block, hidden_states, attention_mask, position_ids, cache_params)
        # # backward pass
        # hidden_states_reverse = self.seq_modeling_block(
        #     hidden_states=hidden_states.flip(1),
        #     attention_mask=attention_mask,
        #     position_ids=position_ids,
        #     cache_params=cache_params,
        # )
        # hidden_states = (hidden_states + hidden_states_reverse.flip(1)) / 2
#         hidden_states = residual + hidden_states
        ...
        residual = hidden_states
        hidden_states = self.ffn_norm(hidden_states)
        hidden_states = self.mlp(hidden_states) #cp(self.mlp, hidden_states) # self.mlp(hidden_states)
        hidden_states = residual + hidden_states
