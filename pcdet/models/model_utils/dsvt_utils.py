...
@torch.no_grad()
def get_inner_win_inds_pytroch(win_inds):
    sort_inds, order = win_inds.sort() #sort_inds is like [0,0,0, 1, 2,2] -> [0,1, 2, 0, 0, 1]
    # sorted_indices = win_inds.argsort()

    check_index = torch.arange(0, len(win_inds)).to(win_inds)
    assert (check_index == order).sum() == len(win_inds)
