...
class PointHeadTemplate(nn.Module):
    ...
    def assign_stack_targets(self, points, gt_boxes, extend_gt_boxes=None,
        ...
        for k in range(batch_size):
            ...
            if ret_part_labels:
                ...
                part_labels = (transformed_points / gt_box_of_fg_points[:, 3:6]) + offset
                point_part_labels_single[fg_flag] = part_labels.clamp(0.0, 1.0)
    ...
    def get_part_layer_loss(self, tb_dict=None):
        ...
        # invalid_mask = (point_part_labels < 0) | (point_part_labels > 1)
        # if invalid_mask.any():
        #     invalid_vals = point_part_labels[invalid_mask].detach().cpu().numpy().tolist()
        #     print("⚠️ point_part_labels contains invalid values outside [0,1]!")
        #     print(f"Count: {len(invalid_vals)} | Min: {min(invalid_vals)} | Max: {max(invalid_vals)}")
        #     print("Examples:", invalid_vals[:20])  # 只展示前20个
        # print("point_part_labels:",point_part_labels.shape,point_part_preds.shape)
