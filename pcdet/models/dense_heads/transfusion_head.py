...
class TransFusionHead(nn.Module):
    ...
    def predict(self, inputs):
        ...
        elif self.dataset_name == "PI3DET_Dataset":
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        heatmap = gt_bboxes_3d.new_zeros(self.num_classes, feature_map_size[0], feature_map_size[1])
    ...
    def encode_bbox(self, bboxes):
        ...
        if self.code_size == 10:
            if bboxes.shape[1] > 7:
                num_cols = min(bboxes.shape[1] - 7, 2)  # 最多取 2 列
                targets[:, 8:8+num_cols] = bboxes[:, 7:7+num_cols]
            # 如果 bboxes 列数 <= 7，targets[:,8:10] 保持为 0
