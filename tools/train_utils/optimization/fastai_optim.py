...
def trainable_params(m: nn.Module, params):
    "Return list of trainable params in `m`."
    if params is not None:
        params = chain(m.parameters(), params)
    else:
        params = m.parameters()
    res = filter(lambda p: p.requires_grad, params)
...
class OptimWrapper():
    ...
    def create(cls, opt_func, lr,
               layer_groups, params=None, **kwargs):
        "Create an `optim.Optimizer` from `opt_func` with `lr`. Set lr on `layer_groups`."
        split_groups = split_bn_bias(layer_groups)

        p1 = {'params': trainable_params(split_groups[0], params), 'lr': 0}
        p2 = {'params': trainable_params(split_groups[1], None), 'lr': 0}
        ...
        if self.true_wd:
            for lr, wd, pg1, pg2 in zip(self._lr, self._wd, self.opt.param_groups[::2], self.opt.param_groups[1::2]):
                for p in pg1['params']:
                    ...
                    if not hasattr(p, '_no_weight_decay') or not p._no_weight_decay:
                        p.data.mul_(1 - wd * lr)
                if self.bn_wd:
                    for p in pg2['params']:
                        # When some parameters are fixed:  Shaoshuai Shi
